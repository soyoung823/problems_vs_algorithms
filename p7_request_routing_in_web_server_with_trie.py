...
For the sake of simplicity we will just use a string that we can print out to ensure we got 
the right handler
...
if we have a lot of pages on our site. A more sensible way to split things would be 
...
on RouteTrieNodes. We really just need to insert and find nodes, 
...
class RouteTrieNode:
    ...
    def insert(self, ...):
        ...
        self.children[char] = self.children.get(char, RouteTrieNode(char))
...
class RouteTrie:
    ...
    def find(self, word):
        ...
        # Find the trie node that represents this word
